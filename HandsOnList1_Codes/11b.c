...
Date:  04-04-2025  
Description: This program opens a file, duplicates the file descriptor using dup2,  appends data to the file using both descriptors, and verifies  whether the file is updated correctly.  
*/  
#include <stdio.h>
#include <stdlib.h>
#include <unistd.h>
...
int main() {
    char f_name[] = "f12_dup2_test.txt";  // Predefined filename
    ...
    int f2 = 5;  // Choose a specific file descriptor number
    if (dup2(f1, f2) < 0) {
        perror("dup2 failed");
        close(f1);
        return 1;
